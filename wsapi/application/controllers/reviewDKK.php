	...
	function index_get() {
        ...
        if ($id == '') {
            ...
            if($id=='page' && $id2!==''){
                ...
                if($id!=='' && $id2==''){
                    $result= $this->simop_kapal_list_DKK_model->get(array('NO_UKK'=>$id));  
                }
            }
        }        
    ...
    function index_post() {
        ...
        if($this->form_validation->run('simop_kapal_list_dkk_post') != false){
            ...
            $safe_data = $this->simop_kapal_list_DKK_model->get(array(
				...
				'TGL_JAM_TIBA'=>$this->post('TGL_JAM_TIBA'),
				'TGL_JAM_BERANGKAT'=>$this->post('TGL_JAM_BERANGKAT')
			));
			//echo $this->post('TGL_JAM_TIBA');
			//die();
			...
			if (($this->post('TGL_AWAL')!='')/*&& ($this->post('END_DATE')!='')*/){
				//$this->kapal_prod_db->where('TGL_JAM_BERANGKAT_PPKB BETWEEN ', '.$this->post('START_DATE').' AND '.$this->post('END_DATE').'' );
				$this->kapal_prod_db->where('TRUNC(TGL_JAM_BERANGKAT_PPKB) BETWEEN ', 'TRUNC(TO_DATE('.$this->post('TGL_JAM_TIBA').'."DD-MON-RR")) 
				AND TRUNC(TO_DATE("'.$this->post('TGL_JAM_TIBA').'","DD-MON-RR"))' );
			}
